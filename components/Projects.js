          ...
          <div key={idx}>
            <a href={project.link} target="_blank" rel="noreferrer">
              <img
                src={project.imgUrl}
                alt={project.title}
                className="rounded-lg shadow-lg w-full h-[500px] object-cover sm:text-white md:text-white"
              />
            </a>
            <h1 className="text-2xl font-bold sm:text-white md:text-white text-center my-5">
              {project.title}
            </h1>
          </div>
        ))}
